...
# no borrower found
my $NO_BORROWER_ERR         = 'NO_BORROWER';
...
my $INVALID_DATE_MAX_ERR    = 'INVALID_DATE_MAX'; # with confirmation
...
if (!C4::Context->userenv){
    ...
    if ($session->param('desk') eq 'NO_DESK_SET'){
        ...
        print $query->redirect("/cgi-bin/koha/desk/selectdesk.pl?oldreferer=/cgi-bin/koha/stack/stack-request.pl".$parameters);
        exit;
    }
} 
...
my ( $template, $loggedinuser, $cookie ) = get_template_and_user(
        {
            template_name   => "stack/stack-request.tmpl",
            query           => $query,
            type            => 'intranet',
            authnotrequired => 0,
...
unless (C4::Context->preference('UseStackrequest')) {
    die "Stack request is disabled in system preferences.";
}
# END MAN296
...
my $doborr       = $query->param('doborr')       || '';
my $from_reserve = $query->param('from_reserve') || '';
...
my $borrowernumber = $query->param('borrowernumber') || undef;
...
unless ($borrowernumber) {
    ...
    #
    # Card number has been posted
    #
    if ($doborr && $cardnumber) {
...
# At this point, borrowernumber exits
unless ($borrower) {
...
my @ignore_istates = ();
if ( $from_reserve && $borrowernumber && $itemnumber ) {
    #Create a stack request from a reserve. We must ignore the reserve first or the item will not be available
    push @ignore_istates, $ISTATE_RES_GUARD;
}
...
if ($itemnumber) {
    ...
    unless ($iteminfo) {
        die "Item not found with itemnumber: ".$itemnumber;
    }
    ...
    unless (CanRequestStackOnItem($itemnumber, \@ignore_istates)) {
        push(@cantrq_loop, {
            $NOT_AVAILABLE_ERR => 1
        });
    }
    ...
    unless (scalar @cantrq_loop) {
        # Physical addresse from OLIMP webservice
        my $marcrecord = C4::Items::_marc_from_item_hash($iteminfo, GetFrameworkCode( $iteminfo->{'biblionumber'}));
        
        $physical_address = $marcrecord->subfield('995','d');
        unless ($physical_address) {
            my $callnumber = $marcrecord->subfield('995','k');
            my $freeAccessCallnumber = $marcrecord->subfield('995','B');
            my $storeCallnumber = $marcrecord->subfield('995','K');
            
            my ($location, $address) = FindLocationAndAddress($callnumber, $freeAccessCallnumber, $storeCallnumber);
...
if (!$onitem && ($biblionumber || $isgeneric)) {
    ...
    if (!$isgeneric) {
        ...
        unless (scalar @$serialinfo) {
            die "No subscription found on biblionumber: ".$biblionumber;
        }
    ...
    # if generic datas will come from item
...
if (scalar @cantrq_loop) {
    ...
    $template->param(
        ...
        # be carefull, some vars are already assigned
        sr_borrowernumber => $borrowernumber,
        sr_firstname      => $firstname,
        sr_surname        => $surname,
        sr_cardnumber     => $cardnumber,
    );
    output_html_with_http_headers $query, $cookie, $template->output;
...
if ($dorequest || $dodesk) {
    ...
    unless (scalar @post_err_loop) {
        ...
        unless ($reserved_space) {
            ...
            if ($asked_begdate_iso lt C4::Dates->today('iso')) {
                ...
                push(@needs_confirm_loop, { $INVALID_DATE_MAX_ERR => 1 });
            }
    ...
    unless (scalar @post_err_loop) {
        ...
        push(@needs_confirm_loop, @$needs_confirm);
    }
    
    if ( !scalar @post_err_loop
     && (!scalar @needs_confirm_loop || $confirmed) ){
        ...
        if (!$must_choose_desk || $dodesk) {
            ...
            # If the stack is created from a hold, complete it before
            my ( $restype, $res ) = C4::Reserves::CheckReserves( $itemnumber );
            if ( $from_reserve && $restype && $borrowernumber eq $res->{'borrowernumber'} ) {
                ModReserveFill($res);
            }
            ...
            if ( $from_reserve && $restype && $borrowernumber eq $res->{'borrowernumber'} ) {
                EditStackRequest( $stack );
                RetrieveStackRequest( $stack, $desk, 1 );
                print $query->redirect('/cgi-bin/koha/stack/all-operations.pl?input=' . $id);
                exit;
            }
        ...
        $template->param(
            ...
            needs_confirm_loop  => \@needs_confirm_loop,
        );
        
        foreach my $needs_confirm (@needs_confirm_loop) {
            if (($needs_confirm->{'TOO_MANY_SR'}    && !$template->{'param_map'}->{'CAN_user_circulate_override_maxstackqty'})
             || ($needs_confirm->{'TOO_MANY_INST'}  && !$template->{'param_map'}->{'CAN_user_circulate_override_maxinstantstackqty'})
             || ($needs_confirm->{'TOO_MANY_DELAY'} && !$template->{'param_map'}->{'CAN_user_circulate_override_maxdelayedstackqty'}))
            {
                $template->param(
                    cant_confirm => 1,
                );
