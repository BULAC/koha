...
my ($template, $borrowernumber, $cookie) = get_template_and_user({
        template_name   => "tools/inventory_cumulatif.tmpl",
        query           => $input,
        type            => "intranet",
        authnotrequired => 0,
...
my @brcditems;
if ($uploadbarcodes && length($uploadbarcodes)>0 && $itembarcode && length($itembarcode)>0){
    my @errorsaisie='1';
...
#   warn "$date";
    ...
    if ($uploadbarcodes && length($uploadbarcodes)>0) {
        while (my $barcode=<$uploadbarcodes>){
            $barcode =~ s/\r?\n$//;
           ($count,@errorloop) = _barcode_loop($date,$qonloan,$qwthdrawn,$count,$barcode,@errorloop);
        }
    }
...
sub _barcode_loop {
    ...
    if ($qwthdrawn->execute($barcode) &&$qwthdrawn->rows){
        ...
        if (defined $item && $item->{'itemnumber'}){
            ModItem({ datelastseen => $date, seencount => $item->{'seencount'} + 1 }, undef, $item->{'itemnumber'});
            push @brcditems, $item;
            $count++;
