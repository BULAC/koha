...
my ( $template, $borrowernumber, $cookie ) = get_template_and_user(
    {
        template_name => "reports/completedspacebookingofday.tmpl",
        query => $input,
        type => "intranet",
        authnotrequired => 0,
...
if ( $op eq 'report' ) {
    ...
    my ( $report_directory, $report_name, $report_action ) = ( 'exports', 'liste_reservations_espace_terminees_a_nettoyer', 'visualization' );
    my @report_errors = ();
...
$template->param(
    report => 'completedspacebookingofday.pl',
