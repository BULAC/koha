...
my ( $template, $borrowernumber, $cookie ) = get_template_and_user(
	{
		template_name => "reports/report_sort.tmpl",
		query => $input,
		type => "intranet",
		authnotrequired => 0,
...
if ( $op eq 'report' ) {
	...
	my ( $report_directory, $report_name, $report_action ) = ( 'exports', 'liste_communications_expirees', 'visualization' );
	my @report_errors = ();
...
$template->param(
	report => 'expiredcommunications.pl',
	expiredcommunication => 1,
