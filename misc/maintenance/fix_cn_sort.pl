...
sub compute_cn_sort($$) {
    my $cn_itemnumber = shift;
    my $cn_itemcallnumber = shift;
    
    my $key;
    
    if ( defined $cn_itemcallnumber ) {
        if ( IsItemInStore( $cn_itemnumber ) ) {
            if ( IsRetroCallnumber( $cn_itemcallnumber ) ) {
                my ( $base, $sequence, $rest ) =  GetBaseAndSequenceFromStoreCallnumber( $cn_itemcallnumber, 0 );
                ...
                my ( $branch, $mention, $type, $format, $number ) = StoreCallnumberCut( $cn_itemcallnumber );
...
my $query = "SELECT itemnumber, itemcallnumber FROM items";
my $sth_query = $dbh->prepare( $query );

my $update = "UPDATE items SET cn_sort = ? WHERE itemnumber = ?";
my $sth_update = $dbh->prepare( $update );

$sth_query->execute();

my $rows = $sth_query->fetchall_arrayref({});

foreach my $row ( @{$rows} ) {
    my $itemnumber = $row->{itemnumber};
    my $itemcallnumber = $row->{itemcallnumber};
    my $cn_sort = compute_cn_sort($itemnumber, $itemcallnumber);
    $sth_update->execute($cn_sort, $itemnumber);
}
