...
sub new {
	...
	my $debarred  = $kp->{debarred}||$kp->{debarred2};  # 1 if ($kp->{flags}->{DBARRED}->{noissues}); B015
	...
	for (qw(EXPIRED CHARGES CREDITS GNA LOST DEBARRED DEBARRED2 DOCS NOTES)) { # B015
		($flags->{$_}) or next;
...
my %fields = (
    ...
    docs                    => 0,   # for patron_status[4] B015
    recall_overdue          => 0,
    currency                => 1,
#   fee_limit               => 0,
    screen_msg              => 1,
