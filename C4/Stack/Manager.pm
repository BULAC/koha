...
sub CreateStackRequest($) {
    ...
    if ($space) {
        ...
        my $max_date = sprintf( "%04d-%02d-%02d", Add_Delta_Days( split(/-/, $fields->{'begin_date'}), 200) );
        my $borrinfo = GetMemberDetails( $fields->{'borrowernumber'}, undef );
        my $borrbranch = $borrinfo->{'branchcode'};
        
        while ( !IsBranchOpen( $borrbranch, $end_date ) && $end_date lt $max_date ) {
            $end_date = sprintf( "%04d-%02d-%02d", Add_Delta_Days( split(/-/, $end_date), 1) );
        }
